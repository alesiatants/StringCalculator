...
func IsValidExpression(expression string) bool {
	...
	if strings.ContainsRune(expressionNoSpaces, 40) || strings.ContainsRune(expressionNoSpaces, 41){
		
		if !regexp.MustCompile(`\d\)`).MatchString(expressionNoSpaces) || !regexp.MustCompile(`\(\d`).MatchString(expressionNoSpaces) {
			return false
		}
		if regexp.MustCompile(`\)\d`).MatchString(expressionNoSpaces) || regexp.MustCompile(`\d\(`).MatchString(expressionNoSpaces) {
			return false
